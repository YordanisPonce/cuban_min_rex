                ...
                ->schema([
                    ...
                    try {
                        ...
                        if ($data['original_file']) {
                            // Ruta REAL del archivo en tu disco local (public)
                            ...
                            // Subir a S3 el BINARIO (no la URL)
                            $stream = fopen($localPath, 'r');
                            Storage::disk('s3')->writeStream($data['original_file'], $stream);
                            if (is_resource($stream))
                                fclose($stream);

                            // Si es ZIP, extraer desde el archivo local (public) y subir cada archivo a S3
