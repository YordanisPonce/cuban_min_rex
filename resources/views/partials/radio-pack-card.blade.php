...
@endphp
<div class="card">
    <img src="{{ $pack->poster ? $pack->poster : ($pack->user->photo ? $pack->user->photo : asset('assets/img/emisora-portada.png')) }}">
    <div class="pack-meta">
        <p>{{$pack->name}}</p>
    </div>
    <div class="pack-links-top">
        $ {{$pack->price}}
    </div>
    <div class="pack-links">
        <div>
            ...
            <div class="d-flex flex-column relative pb-2">
                <a class="btn btn-icon rounded-pill d-flex gap-2" href="{{ route('file.download', $pack->id)}}">
            ...
            @else
                @if (!in_array($pack->id,Cart::get_current_cart()->items ?? []))
                <a class="btn btn-icon rounded-pill d-flex gap-2" href="{{ route('file.add.cart', $pack->id)}}">
                    <i style="width: 20px">
                ...
                @else
                <a class="btn btn-icon rounded-pill d-flex gap-2" href="{{ route('file.remove.cart', $pack->id)}}">
                    <i style="width: 20px">
