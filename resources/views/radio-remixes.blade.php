        ...
        <form id="filter-form" class="filter-dropdowns">
            <select class="filter-dropdown" onChange="document.getElementById('filter-form').submit()" name="genre"
                ...
                @foreach ($genres as $genre)
                    <option {{ request()->get('genre') === $genre["value"] ? 'selected' : '' }}
                        value="{{ $genre["value"] }}"><span><label>Género:</label>
    ...
    <div class="container" data-aos="fade-up" data-aos-delay="300">
        @if ($tracks->count() === 0)
            <div class="empty">
                ...
                <h3>No se han enontrado <span class="text-primary">RESULTADOS</span> que cumplan con los filtros</h3>
            </div>
        @endif
